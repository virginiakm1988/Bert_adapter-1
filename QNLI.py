...
data_path = 'GLUE/QNLI/train.tsv'
df = pd.read_csv(data_path, sep='\t',error_bad_lines=False)

df.columns = [1,'sen1','sen2','label'] #for QQP

test_path = 'GLUE/QNLI/dev.tsv'
df_test = pd.read_csv(test_path, sep='\t',error_bad_lines=False)

df_test.columns = [1,'sen1','sen2','label'] #for QQP
...
class Allen(Dataset): 
    ...
    def __getitem__(self, index):
        if self.mode == 'train':
            ...
            if df_test['label'][index] == 'entailment':
                label = 1
            else:
                label = 0    
        ...
        return input_ids.view(512), attn_mask.view(512), torch.tensor(label, dtype=torch.long)
...
def plotImage(G_losses, name):
    ...
    plt.ylabel("Accuracy")
    plt.savefig('./result/QNLI/'+name+'.png')
