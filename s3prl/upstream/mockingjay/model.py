...
class BertAdapter(nn.Module):
    ...
    def forward(self, x):
        if 'one' in self.adapter_type :
            ...
            x = self.model(x)
            x = y * x
        elif 'mixed' in self.adapter_type :
            x = self.model(x)
        return x
...
class TransformerOutput(nn.Module):
    def __init__(self, config):
        ...
        if 'vector' in sys.argv[1]:
            ...
            self.adapter_serial = BertAdapter(config, config.hidden_size, int(self.botteneck), 'serial')
            self.adapter_parallel = BertAdapter(config, config.intermediate_size, int(self.botteneck), 'parallel')

    def forward(self, hidden_states, input_tensor):

        if "adapter" in sys.argv[1]:
            if 'vector' in sys.argv[2]:
