...
data_path = './GLUE/QQP/train.tsv'
df = pd.read_csv(data_path, sep='\t')

df.columns = [1,2,3,'sen1','sen2','label'] #for QQP

test_path = './GLUE/QQP/dev.tsv'
df_test = pd.read_csv(test_path, sep='\t')

df_test.columns = [1,2,3,'sen1','sen2','label'] #for QQP
...
def plotImageAcc(G_losses, name, acc):
    print('Start to plot!!')
    plt.figure(figsize=(10, 5))
    plt.title("Accuracy During Epoch, acc = {}".format(acc))
    plt.plot(G_losses)
    plt.xlabel("Epoch")
    plt.ylabel("Accuracy")
    plt.savefig('./result/QQP/acc_'+name+'.png')
def plotImage(G_losses, name):
    ...
    plt.ylabel("Accuracy")
    #plt.legend()
    plt.savefig('./result/QQP/'+name+'.png')
...
for epoch in range(15):
    ...
    if epoch == 0:
        print('預計train時間 = ', 70*(end-epoch_start)/60, '分鐘')
    print('=====================================')
    if epoch == 10:
